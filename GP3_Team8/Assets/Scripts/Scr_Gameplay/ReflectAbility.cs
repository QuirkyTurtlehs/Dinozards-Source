    ...
    */
        ...
        */
        ...
        while (counter <= reflectDuration) {
            ...
            if (playerState.IsState(PlayerStates.PossibleStates.IsReflecting)) {
                ...
                // Sphere cast for enemies and obstacles, if 2 hit each other and are both dashing then both should get stunned. 
                ...
                if (reflectCastHits.Length > 0) {
                    ...
                    foreach (RaycastHit reflectHits in reflectCastHits) {
                        ...
                        // If Reflect Hit Player and Not in same team
                        if (reflectHits.transform.gameObject.tag == "Player" && reflectHits.transform.gameObject != player && !playersReflected.Contains(reflectHits.transform.gameObject) && angle < reflectHitAngle && !reflectHits.transform.gameObject.GetComponent<PlayerStates>().IsState(PlayerStates.PossibleStates.IsReflecting) && reflectHits.transform.gameObject.GetComponent<PlayerStates>().inTeam != playerState.inTeam) {
                            ...
                            // Call Hit On Play with 0 damage and Knock back
                            reflectHits.transform.gameObject.GetComponent<PlayerHit>().OnPlayerHit(new Vector3(dirFromPlayerToEnemy.x, reflectKnockbackY, dirFromPlayerToEnemy.z), reflectKnockback, reflectDamage, 0, 0, PlayerHit.HitBy.Reflect, 0.2f, 0, 1, GetComponentInParent<PlayerInput>().playerIndexInt);
                           
                        }

                        // If Two Reflects Hit Each other and Not in same team
                        else if (reflectHits.transform.gameObject.tag == "Player" && reflectHits.transform.gameObject != player && !playersReflected.Contains(reflectHits.transform.gameObject) && angle < reflectHitAngle && reflectHits.transform.gameObject.GetComponent<PlayerStates>().IsState(PlayerStates.PossibleStates.IsReflecting) && reflectHits.transform.gameObject.GetComponent<PlayerStates>().inTeam != playerState.inTeam) {
                            ...
                            reflectHits.transform.gameObject.GetComponent<PlayerHit>().OnPlayerHit(new Vector3(dirFromPlayerToEnemy.x, reflectKnockbackY, dirFromPlayerToEnemy.z), reflectOnReflectKnockback, reflectDamage, 0.1f, 0.05f, PlayerHit.HitBy.Reflect, 0.2f, 0, 1, GetComponentInParent<PlayerInput>().playerIndexInt);

                            // Calls Player Hit on self
                            player.GetComponent<PlayerHit>().OnPlayerHit(new Vector3(dirFromEnemyToPlayer.x, reflectKnockbackY, dirFromEnemyToPlayer.z), reflectOnReflectKnockback, reflectDamage, 0.1f, 0.05f, PlayerHit.HitBy.Reflect, 0.2f, 0, 1);
                        ...
                        // If Reflect Hit a Projectile and its not the players own projectile and Not in same team
                        else if (reflectHits.transform.gameObject.tag == "Projectile" && !player.GetComponentInChildren<ProjectileAbility>().spawnedProjectiles.Contains(reflectHits.transform.gameObject.transform.root.gameObject) && angle < reflectHitAngle && reflectHits.transform.gameObject.GetComponent<Projectile>().owner.gameObject.GetComponent<PlayerStates>().inTeam != playerState.inTeam) {
    ...
    public bool CanActivate() {
        ...
        if (playerState.IsState(PlayerStates.PossibleStates.IsNormal) || playerState.IsState(PlayerStates.PossibleStates.IsDashing)) {
            ...
            if (!playerState.IsState(PlayerStates.PossibleStates.IsDead)) {
