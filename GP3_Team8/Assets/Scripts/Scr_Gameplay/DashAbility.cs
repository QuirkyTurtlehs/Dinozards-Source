    ...
    IEnumerator Dash() {
        ...
        while (Vector3.Distance (player.transform.position, dashTargetLocation) > 0.1f) {
            ...
            if (playerState.IsState(PlayerStates.PossibleStates.IsDashing)) {
                ...
                // Lerps the player toward from start location to dash location, casting for hits along the way. 
                ...
                // If got a Hit when dashing then leaps through them and checks what it was
                if (dashSphereCastHits.Length > 0) {
                    ...
                    foreach (RaycastHit dashHits in dashSphereCastHits) {
                        ...
                        // If Dashed into a Player and Not in same team
                        if (dashHits.transform.gameObject.tag == "Player" && dashHits.transform.gameObject != player && dashHits.transform.gameObject.GetComponent<PlayerStates>().inTeam != playerState.inTeam) {
                            ...
                            if (dashHits.transform.gameObject.GetComponent<PlayerStates>().IsState(PlayerStates.PossibleStates.IsDashing) && !enemiesDashedInto.Contains(dashHits.transform.gameObject)) {
                                ...
                                // Calls Hit on Self when dashing into each other
                                dirFromEnemyToPlayer = (transform.position - dashHits.collider.gameObject.transform.position).normalized;
                                player.GetComponent<PlayerHit>().OnPlayerHit(new Vector3(dirFromEnemyToPlayer.x, knockbackY, dirFromEnemyToPlayer.z), knockbackWhenDashingIntoEachother, dashDamage, dashOnDashCameraShakeDuration, dashOnDashCameraShakeIntensity, PlayerHit.HitBy.Dash, 0.2f, 0, 1);

                                // Calls Hit on Enemy when dashing into each other
                                dirFromPlayerToEnemy = (dashHits.collider.gameObject.transform.position - transform.position).normalized;
                                dashHits.collider.gameObject.GetComponent<PlayerHit>().OnPlayerHit(new Vector3(dirFromPlayerToEnemy.x, knockbackY, dirFromPlayerToEnemy.z), knockbackWhenDashingIntoEachother, dashDamage, 0, 0, PlayerHit.HitBy.Dash, 0.2f, 0, 1, GetComponentInParent<PlayerInput>().playerIndexInt);

                                //Broadcast damage dealt
                                damageDealtInfo(GetComponentInParent<PlayerInput>().playerIndexInt, dashDamage);

                                // Get Middle point between two players that have dashed into each other
                            ...
                            // If Dashing into player that is not dashing
                                ...
                                dirFromPlayerToEnemy = (dashHits.collider.gameObject.transform.position - transform.position).normalized;
                                dashHits.collider.gameObject.GetComponent<PlayerHit>().OnPlayerHit(new Vector3(dirFromPlayerToEnemy.x, knockbackY, dirFromPlayerToEnemy.z), knockbackToApply, dashDamage, 0, 0, PlayerHit.HitBy.Dash, 0.2f, 0, 0, GetComponentInParent<PlayerInput>().playerIndexInt);
                                ...
                                if (gamePlayHandler.activePlayers <= 1)
                                {
                                    timeManager.SlowDownTime(dashOnDashSlowDownTimeScale, dashOnDashSlowDownDuration);
                                } 
                            }
