...
public class Projectile : MonoBehaviour {
    ...
    void GetsAndSetsTarget() {
        ...
        // If Player was in front and Not in same team
        else if (temp[targetWithLowestAngle].gameObject.GetComponent<PlayerStates>().inTeam != owner.GetComponent<PlayerStates>().inTeam)
        ...
        if (projectilephereCastHits.Length > 0) {
            foreach (RaycastHit projectileHits in projectilephereCastHits) {
                ...
                // If Projectile Hit Projectile
                    ...
                    // So the controllers vibrate when two projectiles Hit for the players who shoot the projectiles. 
                    owner.GetComponent<PlayerHit>().OnPlayerHit(new Vector3(0, 0, 0), 0, 0, projectileMergeCameraShakeDuration, projectileMergeCameraShakeIntensity, PlayerHit.HitBy.FireBall, 0.2f, 0, 1);
    ...
    void OnCollisionEnter (Collision collision) {
        ...
        foreach (ContactPoint contactPoint in collision.contacts) {
            ...
            // If Hit Player that is not in own team
            if (contactPoint.otherCollider.gameObject.tag == "Player" && contactPoint.otherCollider.gameObject != owner && contactPoint.otherCollider.gameObject.GetComponent<PlayerStates>().inTeam != owner.GetComponent<PlayerStates>().inTeam) {
                ...
                collision.gameObject.GetComponent<PlayerHit>().OnPlayerHit(new Vector3(dirFromPlayerToEnemy.x, knockbackY, dirFromPlayerToEnemy.z), currentKnockback, currentProjectileDamage, 0, 0, PlayerHit.HitBy.FireBall, 0.2f, 0, 1, owner.GetComponent<PlayerInput>().playerIndexInt);

                damageDealtInfo(owner.GetComponent<PlayerInput>().playerIndexInt, currentProjectileDamage);
